...
void LoadImages(const string &strPathFolder, vector<string> &vstrImageLeft, vector<double> &vTimeStamps);
...
int main(int argc, char **argv)
{
    if(argc < 5)
    {
        cerr << endl << "Usage: ./mono_inertial_d455 path_to_vocabulary path_to_settings path_to_sequence_folder_1 (path_to_image_folder_2 ... path_to_image_folder_N) name_seq" << endl;
        return 1;
    }
...
void LoadImages(const string &strPathFolder, vector<string> &vstrImageLeft, vector<double> &vTimeStamps)
{
    string strPathLeft = strPathFolder + "/color";
    string strPathTimesLeft = strPathFolder +"/times_color.txt";
    ifstream fTimes;
